...
public class CoastLineAgent extends Agent {
	...
	public void act(Tile[][] map) {
		if (tokens > tokenLimit) {
			Coordinate child1Pos = randomBorderPoint(map.length - 1,
					map[0].length - 1, map);
			CoastLineAgent childAgent1 = new CoastLineAgent(child1Pos.x,
					child1Pos.y, this.tokens / 2, tokenLimit);
			Coordinate child2Pos = randomBorderPoint(map.length - 1,
					map[0].length - 1, map);
			CoastLineAgent childAgent2 = new CoastLineAgent(child2Pos.x,
					child2Pos.y, this.tokens / 2, tokenLimit);
			childAgent1.act(map);
			childAgent2.act(map);
		} else {
			while (tokens > 0) {
				coord = randomBorderPoint(map.length - 1, map[0].length - 1,
						map);
				map[coord.x][coord.y]
						.setHeight(map[coord.x][coord.y].height() - 10);
				tokens--;
			}
		}
	}
